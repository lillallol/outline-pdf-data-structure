...
/**
 * @description Mutates in place the provided outline subtree by adding count to each node 
 * that has its count `undefined`.
 * 
 * It returns the count of the root of the subtree.
 * 
 * Take notice that nodes that have no children have count `1` 
 * (something that is not valid according to pdf documentation).
 * 
 * Nodes that are collapsed have negative count (as pdf documentation dictates).
 * 
 * For `i` being `-1` it calculates the count for all outline nodes, and
